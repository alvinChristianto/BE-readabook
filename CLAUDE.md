...
`@PremiumGuard` only gates premium vs. free. Tier-specific features (Audio Only mode, HD quality) are gated differently:

- The FE reads `subscription.plan` from `/auth/me` and enforces Audio Only client-side by showing the feature as disabled for `ksatria_cerita` users.
- The BE does **not** need a separate guard for Audio Only — the audio track is part of the same video file; there is no separate audio-only endpoint.
- If a dedicated HD stream endpoint is added in a future version, apply a `@PlanGuard('penjaga_keajaiban')` guard at that point.
